...
# Time-stamp: <2016-01-20 10:19:02 marine>
...
def seismic_data(Method, N=20):
    ...
    if Method['s_type'] == 'random':
        Point = seismo.random_points(Method)
        ...
        Point = {'type': "spherical", 'depth':data[:, 6], 'latitude':data[:, 4],
                 'longitude':data[:,5], 'angle': 0.}
        In = {'type': "spherical", 'depth':0., 'latitude':data[:, 0], 'longitude':data[:, 1],
              'angle': 0.}
        Out = {'type': "spherical", 'depth':0., 'latitude':data[:, 2], 'longitude':data[:, 3],
               'angle': 0.}
    ...
    Trajectory = {'type': "spherical", 'depth':Ric-r, 'latitude':theta, 'longitude':phi,
                  'angle': Point['angle']}
...
if __name__ == '__main__':
    ...
    for line in range(method['Nlines']):
        ...
        # great circle is not well handle by basemap if it goes out of the map and goes back again. 
        # map_data.drawgreatcircle(Points['in']['longitude'], Points['in']['latitude'],
        #                          Points['out']['longitude'], Points['out']['latitude']) 
        ax.scatter(Points['turning point']['longitude'], Age_average)
        ax.scatter(Points['turning point']['longitude'], Age, c='r')
